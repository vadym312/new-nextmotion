            ...
            {submitStatus === "success" ? (
              <div className="text-center py-8">
                ...
                <h3 className="text-2xl font-semibold text-gray-900 mb-2">
                  {t("starter.hero.success.title")}
                </h3>
                <p className="text-gray-600">{t("starter.hero.success.message")}</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <input
                    ...
                    placeholder={t("starter.hero.form.firstName")}
                    required
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder={t("starter.hero.form.lastName")}
                    required
                    className="px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
                <input
                  ...
                  onChange={handleChange}
                  placeholder={t("starter.hero.form.email")}
                  required
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder={t("starter.hero.form.phone")}
                  required
                  className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
                <input
                  ...
                  onChange={handleChange}
                  placeholder={t("starter.hero.form.clinicName")}
                  required
                  className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
                ...
                >
                  {isSubmitting ? t("starter.hero.form.submitting") : t("starter.hero.form.submit")}
                  <ArrowRight className="w-5 h-5" />
                </button>

                <p className="text-sm text-gray-500 text-center">
                  {t("starter.hero.form.setupTime")}
                </p>

                {submitStatus === "error" && (
                  <p className="text-red-600 text-center">
                    {t("starter.hero.error.message")}
                  </p>
                )}
              </form>
            )}
